...
def parse_scrabble_file(uploaded_file):
    ...
    for line in lines:
        ...
        word_info = {
            'word': clean_word, 'def': definition, 'f': f_hooks, 
            'b': b_hooks, 'prob': prob, 'play': play
        }
    ...
    if st.session_state.display_alpha:
        ...
        if st.session_state.answered:
            ...
            if user_guess == real_count:
                ...
                # Use a flag to ensure streak only increments once per word
                if 'last_word_scored' not in st.session_state or st.session_state.last_word_scored != st.session_state.display_alpha:
                    st.session_state.streak += 1
                    st.session_state.last_word_scored = st.session_state.display_alpha
                    st.rerun()
            else:
